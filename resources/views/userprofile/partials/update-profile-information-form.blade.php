...
<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100" style="color:#44ca89; font-size:23px; font-weight: bold">
    ...
    <form method="post" action="{{ route('userprofile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="name" :value="__('الأسم')" style="font-size: 17px;"/>
            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $user->name)" required autofocus autocomplete="name" />
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <x-input-label for="email" :value="__('البريد الالكتروني')" style="font-size: 17px;" />
        ...
        <div class="flex items-center gap-4">
            <x-primary-button style="background: #44ca89">{{ __('حفظ') }}</x-primary-button>
            ...
            @if (session('status') === 'userprofile-updated')
                <p
                    ...
                    x-init="setTimeout(() => show = false, 2000)"
                     style="color: #44ca89;font-size:20px; display:block;"  >{{ __('تم الحفظ.') }}
                    </p>
            @endif
        </div>
    </form>
